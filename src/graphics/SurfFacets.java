...
public class SurfFacets extends TransformGroup{
	...
	public SurfFacets(Model model, int ir,Color color,double transp){
		...
		if(this.elCode==0) {
			setFacets3ang(model,ir,color,transp);
		//	setConvexHull(model,ir,color,transp);
	...
	public void setConvexHull(Model model, int ir,Color color, double transp){
...
if(ir!=2) return;
	...
	int nNodes0=nn.length;

	Vect[] pp0=new Vect[nNodes0];
	for(int i=0;i<nNodes0;i++)
		pp0[i]=model.node[nn[i]].getCoord();
	
	Vect[] pp=util.getHull(pp0);
	
	int nNodes=pp.length;

	P3f[] coords=new P3f[nNodes];
	for(int j=0;j<nNodes;j++){
		coords[j]=new P3f(pp[j].v3());
...
/*	this.facet3angh.setCoordinates(0, coords);
	...
	//======
			...
			int[] coordIndices=new int[2*this.nSurfEdges];
			int ix=0;
			for(int i=0; i<this.nSurfEdges;i++){

				if(i<this.nSurfEdges-1){
				coordIndices[2*ix]=i;
				coordIndices[2*ix+1]=i+1;
				}else{
					coordIndices[2*ix]=i;
					coordIndices[2*ix+1]=0;
				}
				ix++;
	...
	public void setFacetsPyramid(Model model, int ir,Color color,double transp){
		...
		int[][] edgeLocalNumbQ={{0,1,2,3}};
		int[][] nodeLocalNumbQ={{0,3,2,1}};
		int[][] edgeLocalNumbT={{0,4},{1,5},{2,6},{3,7}};
		int[][] nodeLocalNumbT={{0,1,4},{1,2,4},{2,3,4},{3,0,4}};
...
//======================
	...
	public void setElementField3DK(Model model,ColorBar cBar,int K1){
		...
		for(int i=model.region[nr].getFirstEl();i<=model.region[nr].getLastEl();i++){
			...
			Vect Bunit=B.normalized();

			Mat R=util.rotMat(Bunit, z);
...
public void rescaleElementField3DK(Model model,ColorBar cBar,double a){
	...
	if(N==0) return;
		...
		for(int i=model.region[nr].getFirstEl();i<=model.region[nr].getLastEl();i++){
			...
			Vect Bunit=B.normalized();
			boolean anglePi=(Bunit.dot(z)<-.999);
...
public void rescaleNodalField3D(Model model,ColorBar cBar,double a,boolean all){
 ...
 public void rescaleElementField2D(Model model,ColorBar cBar,double a)  throws NullPointerException{
	...
	if(model.element[1].getB().length==2){
		for(int i=model.region[nr].getFirstEl();i<=model.region[nr].getLastEl();i++){
			...
			edgeColors[ix++]=new Color3f(cBar.getColor(scale).darker());
			 ...
			 R=util.rotMat2D(B.normalized(), y);
		...
		for(int i=model.region[nr].getFirstEl();i<=model.region[nr].getLastEl();i++){
			...
			edgeColors[ix++]=new Color3f(cBar.getColor(scale).darker());
			 ...
			 R=util.rotMat(B.normalized(), z);
	

			Vect P=model.getElementCenter(i);

			for(int j=0; j<K;j++){
				Vect v=R.mul(av[j].v3().times(scale));
				coords[p]=new P3f(P.v3().add(v));		
				p++;
